...
class PokemonMenu
  ...
  def pbStartPokemonMenu
    ...
    commands[cmdBag=commands.length]=_INTL("Zaino") if !pbInBugContest?
    ...
    if pbInSafari?
      ...
      commands[cmdQuit=commands.length]=_INTL("Esci")
    else
      commands[cmdSave=commands.length]=_INTL("Salva") if !$game_system || !$game_system.save_disabled
    end
    commands[cmdOption=commands.length]=_INTL("Opzioni")
    commands[cmdDebug=commands.length]=_INTL("Save Editor") if defined?(XENOVERSE_SAVE_EDITOR_ENABLED) && XENOVERSE_SAVE_EDITOR_ENABLED
    commands[commands.length]=_INTL("Esci")
    loop do
